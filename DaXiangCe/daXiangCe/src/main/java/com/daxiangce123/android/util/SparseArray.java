...
public class SparseArray<E> implements Cloneable {
	...
	public E getByAntiIndex(int index) {
		if (mValues == null) {
			return null;
		}
		index = size() - 1 - index;
	 ...
	 *            whether replacing the previous mapping from the specified key
	 *            if there was one.
	 */
	public void put(long key, E value, boolean override) {
		int i = ContainerHelpers.binarySearch(mKeys, mSize, key);
		if (i >= 0) {
			if (override) {
				mValues[i] = value;
				...
				// n);
				System.arraycopy(mKeys, 0, nkeys, 0, i);
				System.arraycopy(mValues, 0, nvalues, 0, i);

				System.arraycopy(mKeys, i, nkeys, i + 1, mSize - i);
				System.arraycopy(mValues, i, nvalues, i + 1, mSize - i);
	 ...
	 * </p>
	 */
	public long keyAntiAt(int index) {
		if (mGarbage) {
			gc();
		}
		return mKeys[size() - 1 - index];
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public E valueAntiAt(int index) {
		if (mGarbage) {
			gc();
		}
		return (E) mValues[size() - 1 - index];
	}
