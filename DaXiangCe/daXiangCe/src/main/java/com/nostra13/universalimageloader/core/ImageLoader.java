...
public class ImageLoader {
    ...
    public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
        ...
        if (bmp != null && !bmp.isRecycled()) {
            if (App.DEBUG) {
                Log.v(TAG, LOG_LOAD_IMAGE_FROM_MEMORY_CACHE + memoryCacheKey);
            }
            ...
            if (App.DEBUG) {
                Log.v(TAG, "load from disc or net " + memoryCacheKey);
            }
    ...
    /**
     * @param uri see {@link #loadImage(String, ImageLoadingListener)}
     */
    public Bitmap getBitmap(String uri, ImageView imageView) {
        return getBitmap(uri, new ImageViewAware(imageView));
    }

    public Bitmap getBitmap(String uri, ImageAware imageAware) {
        ImageSize targetSize = ImageSizeUtils.defineTargetSizeForView(imageAware, configuration.getMaxImageSize());
        String memoryCacheKey = MemoryCacheUtils.generateKey(uri, targetSize);
        Bitmap bmp = configuration.memoryCache.get(memoryCacheKey);
        if (bmp == null || bmp.isRecycled()) {
