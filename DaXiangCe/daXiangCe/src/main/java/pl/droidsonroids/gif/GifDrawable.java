...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
	...
	private static void runOnUiThread(Runnable task) {
		if (Looper.myLooper() == UI_HANDLER.getLooper()) task.run();
		else UI_HANDLER.post(task);
	}
	...
	public void getPixels(int[] pixels) {
		...
		if (pixels.length < colors.length) throw new ArrayIndexOutOfBoundsException("Pixels array is too small. Required length: " + colors.length);
	...
	public void draw(Canvas canvas) {
		...
		if (mPaint.getShader() == null) {
			if (mIsRunning) renderFrame(mColors, mGifInfoPtr, mMetaData);
			else mMetaData[4] = -1;

			canvas.scale(mSx, mSy);
			final int[] colors = mColors;

			if (colors != null) canvas.drawBitmap(colors, 0, mMetaData[0], 0f, 0f, mMetaData[0], mMetaData[1], true, mPaint);

			if (mMetaData[4] >= 0 && mMetaData[2] > 1) UI_HANDLER.postDelayed(mInvalidateTask, mMetaData[4]);// TODO
																												// don't
																												// post
																												// if
																												// message
	...
	public Bitmap getThumb() {
		// TODO seekToFram may use too many time
		Bitmap thumbBitmap = Bitmap.createBitmap(mMetaData[0], mMetaData[1], Bitmap.Config.RGB_565);
