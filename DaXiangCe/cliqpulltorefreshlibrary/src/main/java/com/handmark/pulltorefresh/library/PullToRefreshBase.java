...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected void onRefreshing(final boolean doScroll) {
		if (mMode.showHeaderLoadingLayout()) {
			Log.d(LOG_TAG, "showHeaderLoadingLayout");
			mHeaderLayout.refreshing();
		}
		if (mMode.showFooterLoadingLayout()) {
			Log.d(LOG_TAG, "showFooterLoadingLayout");
	...
	/**
	 * Actions a Pull Event TODO if pull not work obj here
	 * 
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			case PULL_FROM_END:
				Log.v(LOG_TAG, "mCurrentMode PULL_FROM_END");
				newScrollValue = Math.round(Math.max(initialMotionValue - lastMotionValue, 0) / FRICTION);
				itemDimension = getFooterSize();
				break;
			case PULL_FROM_START:
			default:
				Log.v(LOG_TAG, "mCurrentMode PULL_FROM_START");
				newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION);
				itemDimension = getHeaderSize();
				break;
		}
		Log.v(LOG_TAG, "setHeaderScroll");
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				Log.v(LOG_TAG, "setState(State.PULL_TO_REFRESH);");
				setState(State.PULL_TO_REFRESH);
			} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
				Log.v(LOG_TAG, "setState(State.RELEASE_TO_REFRESH);");
