...
package me.yourbay.barcoder.barcode.commen;
...
public abstract class PlatformSupportManager<T> {
	...
	public T build() {
		for (Integer minVersion : implementations.keySet()) {
			if (Build.VERSION.SDK_INT >= minVersion) {
				...
				try {
					...
					log("Using IMPLEMENTATION " + clazz + " of "
		...
		log("Using DEFAULT implementation " + defaultImplementation.getClass()
				+ " of " + managedInterface);
		return defaultImplementation;
	}

	private void log(String msg) {
		Hyb.log(PlatformSupportManager.class, msg);
	}
